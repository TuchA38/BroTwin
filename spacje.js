document.addEventListener('DOMContentLoaded', function() {
    function replaceSingleLettersWithNbsp(node) {
        // jeśli to węzeł tekstowy — przetwórz tekst
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            // nie ruszamy elementów w których nie chcemy ingerować
            const forbiddenTags = ['A', 'HEADER', 'FOOTER', 'IMG', 'SCRIPT', 'STYLE', 'PRE', 'CODE', 'INPUT', 'TEXTAREA'];
    ...
    function processBodyAndInfo() {
        ...
        if (infoDiv) replaceSingleLettersWithNbsp(infoDiv);
        ...
        if (informacjaDiv) replaceSingleLettersWithNbsp(informacjaDiv);
    }

    function observeInfoDiv() {
        const infoDiv = document.getElementById('info');
        if (infoDiv) {
            const observer = new MutationObserver(() => { replaceSingleLettersWithNbsp(infoDiv); });
    ...
    function observeInformacjaDiv() {
        const informacjaDiv = document.getElementById('informacja');
        if (informacjaDiv) {
            const observer = new MutationObserver(() => { replaceSingleLettersWithNbsp(informacjaDiv); });
            observer.observe(informacjaDiv, { childList: true, subtree: true, characterData: true });
        }
    }
